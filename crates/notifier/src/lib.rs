...
/// If `secret` is Some, adds the legacy `X-TxWatch-Secret` header and a
/// `X-TxWatch-Signature` HMAC header to every request.
///
...
mod tests {
    ...
    async fn fails_after_max_retries() {
        ...
        assert!(msg.contains("HTTP 500"), "error should mention final HTTP error, got: {}", msg);
    }
